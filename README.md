...
**说明**：
- `true`: 系统会正常搜索相关推文并发送新推文/回复推文
- `false`: 系统会跳过所有推文发送和搜索操作，但其他功能（如Webhook通知）仍正常工作

**使用场景**：
- **开发调试**: 临时禁用推文发送以避免在测试期间发送大量推文
- **维护窗口**: 在系统维护期间禁用推文功能
- **成本控制**: 根据需要启用或禁用推文功能来控制API调用成本
